...
advanced_buff_terminal = ScreenPlay:new {
	numberOfActs = 1,
...
registerScreenPlay("advanced_buff_terminal", true)

function advanced_buff_terminal:start()
	-- Spawn terminals
	for i = 1, #self.terminals, 1 do
		...
		if (pTerminal ~= nil) then
			...
			SceneObject(pTerminal):setObjectMenuComponent("advanced_buff_terminalMenuComponent")
...
function advanced_buff_terminal:openWindow(pCreatureObject, pUsingObject)
	local sui = SuiListBox.new("advanced_buff_terminal", "defaultCallback")
...
function advanced_buff_terminal:defaultCallback(pPlayer, pSui, eventIndex, args)
...
function advanced_buff_terminal:removePlayerBuffs(pPlayer)
...
function advanced_buff_terminal:removePetBuffs(pPlayer)
...
function advanced_buff_terminal:healWounds(pPlayer)
...
function advanced_buff_terminal:applyBuff(pPlayer, buffSelected)
...
advanced_buff_terminalMenuComponent = { }

function advanced_buff_terminal:fillObjectMenuResponse(pSceneObject, pMenuResponse, pPlayer)
	local menuResponse = LuaObjectMenuResponse(pMenuResponse)
	menuResponse:addRadialMenuItem(20, 3, "Use Terminal")
	menuResponse:addRadialMenuItem(120, 3, "About Buffs")
end

function advanced_buff_terminal:handleObjectMenuSelect(pObject, pPlayer, selectedID)
	...
	if (selectedID == 20) then
		advanced_buff_terminal:openWindow(pPlayer, pObject)
	elseif (selectedID == 120) then
		...
		for i = 1, #advanced_buff_terminall.buffs, 1 do
			helpMessage = helpMessage .. advanced_buff_terminal.buffs[i][1] .. " (" .. tostring(advanced_buff_terminal.buffs[i][2]) .. "cr/" .. tostring(advanced_buff_terminal.buffs[i][3]) .. "h):\n" -- BuffName (1234cr/2h):
			helpMessage = helpMessage .. "- - - - - - - - - - - - -\n"
			helpMessage = helpMessage .. "Health: " .. advanced_buff_terminal.buffs[i][4] .. "\n"
			helpMessage = helpMessage .. "Strength: " .. advanced_buff_terminal.buffs[i][5] .. "\n"
			helpMessage = helpMessage .. "Constitution: " .. advanced_buff_terminal.buffs[i][6] .. "\n"
			helpMessage = helpMessage .. "Action: " .. advanced_buff_terminal.buffs[i][7] .. "\n"
			helpMessage = helpMessage .. "Quickness: " .. advanced_buff_terminal.buffs[i][8] .. "\n"
			helpMessage = helpMessage .. "Stamina: " .. advanced_buff_terminal.buffs[i][9] .. "\n"
			helpMessage = helpMessage .. "Mind: " .. advanced_buff_terminal.buffs[i][10] .. "\n"
			helpMessage = helpMessage .. "Focus: " .. advanced_buff_terminal.buffs[i][11] .. "\n"
			helpMessage = helpMessage .. "Willpower: " .. advanced_buff_terminal.buffs[i][12] .. "\n\n"
		end
		
		suiManager:sendMessageBox(pObject, pPlayer, "About Buffs", helpMessage, "@ok", "advanced_buff_terminalMenuComponent", "noCallback")
	end 
	
	return 0
end

function advanced_buff_terminalMenuComponent:noCallback(pPlayer, pSui, eventIndex)
	-- do nothing
end
